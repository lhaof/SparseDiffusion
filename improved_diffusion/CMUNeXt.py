...
class CMUNeXt(nn.Module):
    def __init__(self, input_channel=1, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        ...
        # self.Upf5 = nn.Upsample(scale_factor=16, mode='bilinear')
    ...
    def forward(self, x):
        ...
        x2 = self.Maxpool(x1)
        x2 = self.encoder2(x2)
        # feats.append(self.Upf2(x2))
        x3 = self.Maxpool(x2)
        x3 = self.encoder3(x3)
        # feats.append(self.Upf3(x3))
        x4 = self.Maxpool(x3)
        x4 = self.encoder4(x4)
        # feats.append(self.Upf4(x4))
        x5 = self.Maxpool(x4)
        x5 = self.encoder5(x5)
        # feats.append(self.Upf5(x5))

        d5 = self.Up5(x5)# ([4, 64, 32, 32])
        # print("d5Up5.shape",d5.shape) 
        d5 = torch.cat((x4, d5), dim=1)
        d5 = self.Up_conv5(d5) # ([4, 64, 32, 32])
        # print("d5.shape",d5.shape)

        d4 = self.Up4(d5) # ([4, 32, 64, 64])
        feats.append(self.Upf3(d4))
        d4 = torch.cat((x3, d4), dim=1)
        d4 = self.Up_conv4(d4)# ([4, 32, 64, 64])
        feats.append(self.Upf3(d4))

        d3 = self.Up3(d4) # ([4, 16, 128, 128])
        feats.append(self.Upf2(d3))
        ...
        d1 = self.Conv_1x1(d2)
        # end_time = time.time()
        # print(f'time for rrdb:{end_time-start_time:.4f}s.')
...
def cmunext_s(dims=[8, 16, 32, 64, 128], depths=[2, 1, 1, 1, 1], kernels=[3, 3, 7, 7, 9]):
    return CMUNeXt(dims=dims, depths=depths, kernels=kernels)
