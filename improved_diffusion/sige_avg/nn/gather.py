...
class Gather(SIGEModule):
    def __init__(
        ...
        n0 = max(block_size[0] - conv.kernel_size[0], 0) // conv.stride[0]  # (16-3)//1=13
        n1 = max(block_size[1] - conv.kernel_size[1], 0) // conv.stride[1]
        b0 = n0 * conv.stride[0] + conv.kernel_size[0]  # 13*1+3=16
        ...
        self.block_stride = ((n0 + 1) * conv.stride[0], (n1 + 1) * conv.stride[1])  # (14, 14)
        if offset is None:
    ...
    def set_mask(self, masks: Dict, cache: Dict, timestamp: int):
        if self.timestamp != timestamp:
            ...
            if active_indices is None:
                ...
                print("active_indices:",active_indices.shape,active_indices)# torch.Size([817, 2])
