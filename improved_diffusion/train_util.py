...
class TrainLoop:
    ...
    def _load_and_sync_parameters(self, logs_path):
        ...
        # )
        logger.log(f"model folder path")  # 原来是这个
        # if logs_path:
    ...
    def _load_optimizer_state(self, logs_path):
        ...
        opt_checkpoint = bf.join(
            bf.dirname(main_checkpoint), f"all_optsavedmodel{self.resume_step:06}.pt"
        )
    ...
    def run_loop(self, max_iter=250000, start_print_iter=100000, number_of_generated_instances=9):
        ...
        while (
            ...
            batch, cond, _ = next(self.data)  # torch.Size([4, 1, 256, 256],-1~1)  dict_keys(['number_of_annotators', 'conditioned_image'])  tensor([2, 2, 1, 3], dtype=torch.int32)  torch.Size([4, 1, 256, 256])
            # import pdb
            # pdb.set_trace()
            ...
            if self.step>0 and self.step % self.save_interval == 0: #and  or self.step == 60000:
                self.saveall()
                logger.log(f"save model for checkpoint")
                self.save_state_dict()
                dist.barrier()
                ...
                if self.run_without_test:
                    ...
                    self.writer.add_scalar('Loss/validmiou', ema_val_miou, self.step)
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            self.writer.add_scalar('Loss/batchloss', loss.item(), self.step)
            
            
            self.writer.add_scalar('Loss/mskloss', (losses["msk"].mean()).item(), self.step)
            if "vb" in losses:
                self.writer.add_scalar('Loss/vbloss', (losses["vb"].mean()).item(), self.step)
            # self.writer.add_scalar('Loss/vbloss', (losses["vb"].mean()).item(), self.step)
            self.writer.add_scalar('Loss/mseloss', (losses["mse"].mean()).item(), self.step)
            if self.use_fp16:
    ...
    def saveall(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                logger.log(f"saving model {rate}...")
                if not rate:
                    filename = f"all_savedmodel{(self.step+self.resume_step):06d}.pt"
                else:
                    filename = f"all_emasavedmodel_{rate}_{(self.step+self.resume_step):06d}.pt"
                with bf.BlobFile(bf.join(get_blob_logdir(), filename), "wb") as f:
                    th.save(state_dict, f)
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join(get_blob_logdir(), f"all_optsavedmodel{(self.step+self.resume_step):06d}.pt"),
                "wb",
            ) as f:
                th.save(self.opt.state_dict(), f)
...
def parse_resume_step_from_filename(filename):
    ...
    """
    split = filename.split("all_savedmodel")
    if len(split) < 2:
        return 0
...
def find_ema_checkpoint(main_checkpoint, step, rate):
    if main_checkpoint is None:
        return None
    filename = f"all_emasavedmodel_{rate}_{(step):06d}.pt"
