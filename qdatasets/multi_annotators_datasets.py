...
from qdatasets.transforms import Compose, Resize, RandomHorizontalFlip, ToTensor, Normalize, RandomVerticalFlip, RandomAffine
from improved_diffusion import logger

def load_data(
    *, data_dir, batch_size, image_size, dataset_class, deterministic=False, erosion=False, soft_label_training=False, consensus_training=False, no_annotator_training=False,
):
    """
...
class MultiAnnotatorsDataset(Dataset, ABC):
    ...
    def __init__(self, data_dir, mode, no_aug=False,
        ...
        dataset_path = Path(__file__).absolute().parent.parent.parent / f"{data_dir}/{self.get_dataset_name()}"
        self.soft_labeling = soft_label_gt  # t
        self.number_of_annotators = self.get_number_of_annotators()
        self.erosion = erosion  #   f

        mean_list = []
        std_list = []
        self.case_name_to_image_dict = self.get_train_case_name_to_image(dataset_path)
        self.case_name_to_mask_dict_soft = {}
        ...
        elif self.mode == 'val' or self.mode=='dmd':
        ...
        else:  # self.mode == 'trainval'
            self.case_name_to_image_dict = self.get_trainval_case_name_to_image(dataset_path)
            self.case_name_to_mask_dict = self.get_trainval_case_name_to_masks(dataset_path)
        ...
        if self.consensus:
            for case_name, masks in self.case_name_to_mask_dict.items():
                ...
                self.case_name_to_mask_dict_soft[case_name] = np.sum(masks, axis=0) / self.number_of_annotators

        # soft labeling
        elif self.soft_labeling:
            for case_name, masks in self.case_name_to_mask_dict.items():
                self.case_name_to_mask_dict[case_name] = np.sum(masks, axis=0) / self.number_of_annotators  # (h,w) unique([0.        , 0.33333333, 0.66666667, 1.        ])
                self.case_name_to_mask_dict_soft[case_name] = np.sum(masks, axis=0) / self.number_of_annotators
        else:
    ...
    def __getitem__(self, item):
        ...
        img = self.case_name_to_image_dict[image_name]  # (497, 497)
        mask = self.case_name_to_mask_dict[image_name]  # (3, 497, 497)
        masksoft = self.case_name_to_mask_dict_soft[image_name]
        # import pdb
        # pdb.set_trace()
        ...
        # print("--------",masksoft.shape,masksoft.min(),masksoft.max())
        out_dict = {}
        # if self.erosion:
        ...
        if len(img.shape) == 2:
            ...
            # print("****",mask.shape,mask.min(),mask.max())
            img, mask, masksoft = self.transformations(img.astype('float32'), mask.astype('float32'), masksoft.astype('float32'))
        else:
            img, mask, masksoft = self.transformations(img.astype('float32'), mask.astype('float32'), masksoft.astype('float32'))
        ...
        # print("--------",masksoft.shape,masksoft.min(),masksoft.max())
        mask = (2 * mask - 1.0)
        out_dict["softmask"] = masksoft
        
        return mask, out_dict, str(self.image_names[item])  # val:{mask.Size([1, 256, 256],(-1,1)),img([1, 256, 256],(-1,1)),'case21'}
